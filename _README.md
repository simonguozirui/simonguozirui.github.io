...
This repo includes **`.github/workflows/jekyll-pages.yml`**. It installs gems from `Gemfile.lock` and runs `bundle exec jekyll build` — **Jekyll 4 + Dart Sass**, same as local. **Do not** use GitHub’s suggested **`actions/jekyll-build-pages`** workflow: that runs **Jekyll 3.10** and ignores this repo’s locked Jekyll/Ruby toolchain.
